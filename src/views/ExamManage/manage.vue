<template>
  <Row style="margin: 20px">
    ...
    <Breadcrumb>
      ...
      <BreadcrumbItem>试卷发布</BreadcrumbItem>
    </Breadcrumb>
    <!--操作按钮-->
    ...
    <Row style="margin-top: 20px" v-if="selectCourse.trim() !== ''">
      <Row>
        <Col span="2">
          <Button type="primary" icon="ios-cloud-done-outline" @click="beforeAdd">发布试卷</Button>
        </Col>
      </Row>
      <Row style="margin-top: 10px">
        <Col span="24">
          <Table border :columns="examTable" :data="$store.state.examManage.examData">
    ...
    <Modal v-model="addModelState" @on-visible-change="modelWatchHandler(addModelState)"  fullscreen title="公告操作">
      ...
      <div slot="footer">
        <Button type="primary" size="large" :loading="$store.state.examManage.saveLoding" long @click="save(updateModelState ? 'update':'save')">保存</Button>
      </div>
    </Modal>
  ...
  export default {
    ...
    methods: {
      ...
      save(value) {
        ...
        if(value === 'save') {
          this.$store.dispatch("examManage/doSave", {"course": this.selectCourse, "resource": this.modelForm.resourse, "title": this.modelForm.title, "content": this.modelForm.content, "startTime": this.modelForm.date[0], "endTime": this.modelForm.date[1]})
        }
        if(value === 'update') {
          this.$store.dispatch("examManage/doUpdate", {"id": this.modelForm.id, "course": this.selectCourse, "resource": this.modelForm.resourse, "title": this.modelForm.title, "content": this.modelForm.content, "startTime": this.modelForm.date[0], "endTime": this.modelForm.date[1]})
        }
      },
      // 更新
      beforeUpdate(index) {
        let data = this.$store.state.examManage.examData[index];
      ...
      remove(index) {
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            this.$store.dispatch("examManage/doRemove", {"id": data.id, "course": data.course})
          }
        })
      },
      // 查看
